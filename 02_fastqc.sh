...
#$ -o /data/users/sborrego/BioinformaticsSG/Review/griffith_analysis_demo/fastqc_out.out  # contains what would normally be printed to stdout (the$
#$ -e /data/users/sborrego/BioinformaticsSG/Review/griffith_analysis_demo/fastqc_out.err  # file name to print standard error messages to. These m$
...
DATA_FILENAMES=/data/users/sborrego/BioinformaticsSG/Review/griffith_analysis_demo/data/HBR_data_filenames.txt
...
# Here we are performing a loop that will fine each file in our data_filenames.txt file.
...
for FILE in $(cat ${DATA_FILENAMES}); do
    fastqc $FILE \
    --outdir ${QC_OUT_DIR}
